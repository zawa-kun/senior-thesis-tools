...
| DMIS段階 | 翻訳者の意図（読者への要求） | 典型的な翻訳処理 |
| :--- | :--- | :--- |
| **1. 否認 (Denial)** | 文化差を認識させない（重要度が低い、または訳出不能） | **削除**、完全な省略。痕跡を残さない。 |
| **2. 防衛 (Defense)** | 文化要素を「異質なもの」「奇妙なもの」として否定的に際立たせる | 否定的なニュアンスへの変更、過度な異国情緒の強調。（※通常の翻訳では稀） |
| **3. 最小化 (Minimization)** | 文化差を意識させず、物語の普遍性・スムーズな理解を優先する | **一般化**、**等価**。普遍的な上位概念（Peopleなど）や、自文化の類似概念への置き換え。 |
| **4. 受容 (Acceptance)** | 文化差（日本特有の概念）があることを明示し、そのまま受け止めさせる | **借用**、**直訳**。不自然でも原語の響きや構造を残す（Sensei, Futonなど）。 |
| **5. 適応 (Adaptation)** | ターゲット読者の文化視点・感覚に合わせて、機能的に「同じ体験」ができるようにする | **翻案**、**増幅（説明加筆）**、**記述**。補足説明を加えたり、ターゲット文化で同等の機能を持つものに作り変える。 |
| **6. 統合 (Integration)** | 原文とターゲット文化、両方の視点を架橋し、多角的な理解を促す | **訳注**の付与、括弧書きでの併記、造語による概念再構築など、単一の訳語に留まらない処理。 |
