...
# --- ファイルパスの設定 ---
# INPUT_CSV_PATH = Path(__file__).parent.parent / "data" / "processed" / "alignment_ineko.csv"  # 入力ファイル
INPUT_CSV_PATH = "alignment_ineko.csv"
# INPUT_CSV_PATH = "test_data.csv" # test
# OUTPUT_CSV_PATH = Path(__file__).parent.parent / "data" / "processed" / "dmis_ineko.csv"  # 出力ファイル
...
2. キーワード（文化的要素）の扱いに基づき、
   翻訳者が意図的に“どの DMIS モード（否認〜統合）を採用したか”を1つ選択する。
【DMISの翻訳適用（翻訳者が“演じる”文化モード）】
    - 否認：文化差を無視。差異を処理しない（例：文化語の削除）
    - 防衛：他文化を歪めて自文化側に寄せる
    - 最小化：差異を小さく扱い自文化の共通点に収束（例：等価）
    - 受容：差異を認めたうえで保持する（例：借用・直訳）
    - 適応：他文化視点に寄せた構造変換（例：調整・翻案）
    - 統合：両文化を自由に行き来し柔軟に操作（例：文脈に応じた使い分け）
...
例：
借用,受容,原文「畳」を"tatami"で保持し文化差を残すため受容段階を演じて訳出した。
...
def parse_response(response_text):
    ...
    try:
        ...
        if len(parts) >= 3:
            ...
            note = parts[2].strip()
            return method, sensitivity, note
        else:
            # 形式が不正な場合
...
def main():
    ...
    for index, row in tqdm(df.iterrows(), total=total_rows, desc="処理中"):
        ...
        # 回答をパース
        method, sensitivity, remark = parse_response(response)
